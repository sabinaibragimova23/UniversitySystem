...
public class ManagerView extends BaseView {
    ...
    public static void run(Manager manager) throws IOException {
        ...
        while (active) {
            ...
            switch (option) {
                ...
                case 0  -> { System.out.println("[Manager] " + AppContext.tr("logout") + "."); active = false; }
    ...
    private static void menu() {
        ...
        System.out.println(AppContext.tr("managerMenu"));

        System.out.println("1  - " + AppContext.tr("courses"));
        System.out.println("2  - " + AppContext.tr("addCourse"));
        System.out.println("3  - " + AppContext.tr("assignCourse"));
        System.out.println("4  - " + AppContext.tr("studentsByGpa"));
        System.out.println("5  - " + AppContext.tr("studentsByName"));
        System.out.println("6  - " + AppContext.tr("teachers"));
        System.out.println("7  - " + AppContext.tr("report"));
        System.out.println("8  - " + AppContext.tr("requests"));
        System.out.println("9  - " + AppContext.tr("publishNews"));
        System.out.println("10 - " + AppContext.tr("newsList"));
        System.out.println("11 - " + AppContext.tr("createOrg"));
        System.out.println("12 - " + AppContext.tr("addComment"));
    ...
    private static void addCourse() throws IOException {
        System.out.print("Course ID: "); String id = reader.readLine().trim();
        System.out.print("Name: ");      String name = reader.readLine().trim();
        System.out.println("Type: 1-MAJOR 2-MINOR 3-FREE_ELECTIVE");
        int t = Integer.parseInt(reader.readLine().trim());
        CourseType type = switch (t) { case 2 -> CourseType.MINOR; case 3 -> CourseType.FREE_ELECTIVE; default -> CourseType.MAJOR; };
        System.out.print("Credits: "); int credits = Integer.parseInt(reader.readLine().trim());
    ...
    private static void assignCourse(Manager manager) throws IOException {
        ...
        if (courses.isEmpty()) { errorMsg("No courses available. Add a course first."); return; }
        ...
        java.util.List<Teacher> teachers = DataStorage.getUsers().stream()
                .filter(u -> u instanceof Teacher).map(u -> (Teacher) u).collect(java.util.stream.Collectors.toList());
        if (teachers.isEmpty()) { errorMsg("No teachers registered yet."); return; }
        System.out.println("TEACHERS");
        for (int i = 0; i < teachers.size(); i++)
            System.out.println("  " + (i + 1) + " - " + teachers.get(i).getFirstName() + " " + teachers.get(i).getLastName() + " (" + teachers.get(i).getPosition() + ")");
        int ti = readIntRange("Pick teacher: ", 1, teachers.size());
        CourseController.assignToTeacher(course, teachers.get(ti - 1), manager);
        successMsg("Assigned.");
    }
    ...
    private static void publishNews(Manager manager) throws IOException {
        ...
        if (t == 1) System.out.println("[PINNED] Research news published!");
        successMsg("News published.");
    }

    private static void createOrgMenu() throws IOException {
        String name = readString("Organization name: ");
        model.users.StudentOrganization org = new model.users.StudentOrganization(name);
    ...
    private static void addCommentMenu() throws IOException {
        ...
        for (int i = 0; i < newsList.size(); i++) System.out.println("  " + (i + 1) + " - " + newsList.get(i).getTitle());
