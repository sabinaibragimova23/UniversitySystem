...
public class TeacherView extends BaseView {
    ...
    public static void run(Teacher teacher) throws IOException {
        ...
        while (running) {
            ...
            switch (option) {
                ...
                case 10 -> System.out.println("Rating: " + String.format("%.1f", teacher.getRating()));
    ...
    private static void menu() {
        ...
        System.out.println(AppContext.tr("teacherMenu"));

        System.out.println("1  - " + AppContext.tr("myCourses"));
        System.out.println("2  - " + AppContext.tr("putMark"));
        System.out.println("3  - " + AppContext.tr("studentsList"));
        System.out.println("4  - " + AppContext.tr("sendMessage"));
        System.out.println("5  - " + AppContext.tr("sendComplaint"));
        System.out.println("6  - " + AppContext.tr("simpleReport"));
        System.out.println("7  - " + AppContext.tr("detailedReport"));
        System.out.println("8  - " + AppContext.tr("research"));
        System.out.println("9  - " + AppContext.tr("recommendation"));
        System.out.println("10 - " + AppContext.tr("rating"));
        System.out.println("11 - " + AppContext.tr("submitRequest"));
    ...
    private static void putMarkMenu(Teacher teacher) throws IOException {
        ...
        if (myCourses.isEmpty()) { errorMsg("You have no assigned courses."); return; }
        System.out.println("My Courses:");
        for (int i = 0; i < myCourses.size(); i++) System.out.println((i + 1) + " - " + myCourses.get(i).getName());
    ...
    private static void sendMessageMenu(Teacher teacher) throws IOException {
        List<User> allUsers = DataStorage.getUsers().stream()
                .filter(u -> u.getLogin() != null && !u.getLogin().equals(teacher.getLogin())).collect(Collectors.toList());
    ...
    private static void sendComplaintMenu(Teacher teacher) throws IOException {
        ...
        if (students.isEmpty()) { errorMsg("No students in the system."); return; }
        System.out.println("STUDENTS:");
        for (int i = 0; i < students.size(); i++)
            System.out.println((i + 1) + " - " + students.get(i).getFirstName() + " " + students.get(i).getLastName());
        int si = readIntRange("Pick student: ", 1, students.size());
    ...
    private static void detailedReportMenu(Teacher teacher) throws IOException {
        ...
        if (course == null) { errorMsg("Course not found: " + cid); return; }
    ...
    private static void researchMenu(Teacher teacher) throws IOException {
        ...
        switch (opt) {
            ...
            case 8 -> { int year = readInt("Year: "); ResearchController.showTopCitedByYear(year); }
        }
    }
    ...
    private static void writeRecommendationMenu(Teacher teacher) throws IOException {
        ...
        RecommendationLetter letter = teacher.writeRecommendation(student, purpose, body);
        letter.print();
    }
